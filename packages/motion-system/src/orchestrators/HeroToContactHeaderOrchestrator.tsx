...
> = ({ siteConfig }) => {
  ...
  useGSAP(() => {
    ...
    // Poll for elements to be available
    let pollCount = 0
    ...
    const checkElements = () => {
      ...
      const heroNameRef = getElement('hero-name')
      const heroTitleRef = getElement('hero-title')
      const heroEmailRef = getElement('hero-email')
      const heroInstitutionRef = getElement('hero-institution')
      const heroContactRef = getElement('hero-contact')

      // Debug logging
      if (pollCount === 1) {
        console.log('[HeroToContactHeader] First poll - refs:', {
          heroNameRef,
          heroTitleRef,
          heroEmailRef,
        })
      ...
      const heroEmail = heroEmailRef?.current
      const heroInstitution = heroInstitutionRef?.current
      const heroContact = heroContactRef?.current

      if (!heroName || !heroTitle || !heroEmail) {
        if (pollCount >= maxPolls) {
          console.error(
            '[HeroToContactHeader] Failed to find elements after 5 seconds'
          )
          return
        }

        // Log only every 10 attempts to reduce console spam
        if (pollCount % 10 === 1) {
          console.log(
            `[HeroToContactHeader] Waiting for elements to register (attempt ${pollCount}/${maxPolls}):`,
            {
              heroName: !!heroName,
              heroTitle: !!heroTitle,
              heroEmail: !!heroEmail,
        ...
        pollTimeout = setTimeout(checkElements, 100)
        return
      }

      console.log(
        `[HeroToContactHeader] Elements found after ${pollCount} attempts`
      )

      // Elements are ready, continue with setup
      setupOrchestrator({
        heroName,
        heroTitle,
        heroEmail,
        heroInstitution: heroInstitution || undefined,
        heroContact: heroContact || undefined,
      })
    }

    const setupOrchestrator = (elements: {
      heroName: HTMLElement
      heroTitle: HTMLElement
      heroEmail: HTMLElement
      heroInstitution: HTMLElement | undefined
      heroContact: HTMLElement | undefined
    }) => {
      const { heroName, heroTitle, heroEmail, heroInstitution, heroContact } =
        elements
      ...
      // Expose for testing
      ;(
        window as { testGotoSection?: (section: string) => void }
