...
async function globalTeardown(_config: FullConfig) {
  ...
  try {
    ...
    console.log(
      `[Coverage Teardown] Found ${coverageFiles.length} coverage files`
    )
    ...
    console.log(
      `[Coverage Teardown] Merged ${allCoverage.length} coverage entries`
    )
    console.log(
      `[Coverage Teardown] Final coverage saved to ${finalCoverageDir}`
    )
    ...
    console.log(
      `[Coverage Teardown] Overall coverage: ${percentage}% (${coveredBytes}/${totalBytes} bytes)`
    )
  } catch (error) {
