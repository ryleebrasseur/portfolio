...
test.describe('Custom Cursor', () => {
  ...
  test('should change state on hover over interactive elements', async ({
    ...
    await logger.logAction('Waiting for project buttons to be visible')
    ...
    await logger.logAction('Hovering over project button')
    ...
    // Move away
    await logger.logAction('Moving mouse away from button')
    await page.mouse.move(50, 50)
    await page.waitForTimeout(100)
