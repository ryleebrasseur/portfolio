...
test.describe('Custom Cursor', () => {
  ...
  test('should hide default cursor and show custom cursor', async ({ page }) => {
    ...
    const cursor = await page.locator('[class*="cursor"]:not([class*="cursorDot"])')
  ...
  test('should follow mouse movement', async ({ page }) => {
    ...
    // Get cursor position
    const transform1 = await cursor.evaluate(el => 
    ...
    const transform2 = await cursor.evaluate(el => 
      window.getComputedStyle(el).transform
    )
  ...
  test('should change state on hover over interactive elements', async ({ page }) => {
    const cursor = await page.locator('[class*="cursor"]:not([class*="cursorDot"])')
  ...
  test('should handle hovering over non-element targets', async ({ page, browserName }) => {
    ...
    try {
      ...
      if (browserName === 'webkit' && error.message.includes('Page closed')) {
        console.warn('WebKit page closed during mouse movement - known WSL issue')
        // Still check for the actual error we care about
        const typeErrors = errors.filter(e => e.includes('matches is not a function'))
