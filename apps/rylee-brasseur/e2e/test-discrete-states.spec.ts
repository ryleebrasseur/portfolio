...
test('validate discrete state transitions', async ({ page, logger }) => {
  ...
  await logger.logState('Hero opacity after header transition', { opacity: heroOpacity })
  ...
  await logger.logState('Hero opacity after return', { opacity: heroOpacityAfterReturn })
  ...
  await logger.logState('Header opacity after return', { opacity: headerOpacityAfterReturn })
  ...
  await logger.logAction('Test completed - check artifacts for full browser console logs')
})