...
test('Observer pattern responds to actual scroll events', async ({ page, logger }) => {
  ...
  const headerHiddenAgain = await page.locator('#sticky-header-container').evaluate(el => {
    const styles = window.getComputedStyle(el)
    return styles.opacity === '0' || styles.visibility === 'hidden'
  })
