...
test.describe('Hero Section with WebGL', () => {
  ...
  test('should have proper scroll height based on projects', async ({ page }) => {
    const heroContainer = await page.locator('[class*="heroContainer"]')
    const containerHeight = await heroContainer.evaluate(el => el.scrollHeight)
  ...
  test('should be responsive', async ({ page }) => {
    ...
    let titleFontSize = await title.evaluate(el => 
    ...
    titleFontSize = await title.evaluate(el => 
      window.getComputedStyle(el).fontSize
    )
