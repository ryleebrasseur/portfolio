...
const version =
  process.argv[2] || run('git describe --tags --abbrev=0', true) || 'v1.0.0'
...
const lastTag =
  run('git describe --tags --abbrev=0 HEAD^', true) || 'initial commit'
const commitCount = run(`git rev-list ${lastTag}..HEAD --count`, true) || '0'
const commits =
  run(`git log --pretty=format:"- %s" ${lastTag}..HEAD`, true) ||
  'No new commits'

// Discover apps and packages
const apps = existsSync('apps')
  ? readdirSync('apps').filter((d) =>
      existsSync(join('apps', d, 'package.json'))
    )
  : []
const packages = existsSync('packages')
  ? readdirSync('packages').filter((d) =>
...
const appDetails = apps
  .map((app) => {
    const appPkg = JSON.parse(
      readFileSync(join('apps', app, 'package.json'), 'utf-8')
    )
...
if (devDeps['@playwright/test'])
  keyDeps.push(`Playwright ${devDeps['@playwright/test']}`)
