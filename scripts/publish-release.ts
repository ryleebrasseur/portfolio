...
function getRepoInfo(): { owner: string; repo: string } {
  ...
  throw new Error('Could not determine repository owner/name. Set repository in package.json or ensure git remote is configured.');
}
...
function createArtifacts() {
  ...
  if (existsSync('playwright-report') && !existsSync('playwright-report.tar.gz')) {
...
async function uploadAsset(uploadUrl: string, filePath: string) {
  ...
  if (isDryRun) {
    console.log(`  🔍 Would upload: ${filename} (${(fileSize / 1024 / 1024).toFixed(2)} MB)`);
    return;
  }
