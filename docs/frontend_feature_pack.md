...
const ImagePlane = ({ texture, args, scrollData, index, totalProjects,...props }) => {
  ...
  useFrame(() => {
    if (meshRef.current && scrollData) {
      ...
      const projectScrollProgress = (scrollData.current / viewport.height) % totalProjects;
      ...
      meshRef.current.material.opacity = Math.max(0, 1 - distanceToCurrent * 0.5); // Fade out distant planes
...
const HeroSectionWebGLScrollNarrative = () => {
  ...
  return (
    <div className={styles.webglCanvasContainer} style={{ height: `${projects.length * 100}vh` /* Example scroll length */ }}>
      <Canvas camera={{ position: , fov: 75 }}>
        <Suspense fallback={null}> {/* Fallback for texture loading */}
...
test.describe('Hero Section with WebGL Scroll Narrative', () => {
  ...
  test('should respond to scroll interactions', async ({ page }) => {
    ...
    const initialOverlayContent = await page.locator('.overlayContent').textContent(); // Assuming overlay content changes

    // Simulate scroll
    await page.mouse.wheel(0, 500); // Scroll down
    await page.waitForTimeout(1000); // Wait for animations

    const scrolledOverlayContent = await page.locator('.overlayContent').textContent();
  ...
  test('should maintain accessibility for overlay content', async ({ page }) => {
...
*(A GIF demonstrating the scroll-driven WebGL animations, 3D element transitions, and overall narrative flow would be placed here. This would showcase the intended visual output and interaction.)*
...
const ProjectCard = ({ project }) => {
  const { title, category, imageUrl, description, tldrLink, detailsLink } = project;
...
test.describe('Project Showcase Grid/List', () => {
  ...
  test('should render the project showcase title and grid', async ({ page }) => {
    const title = await page.locator('h2:has-text("What I’ve been up to recently...")');
  ...
  test('should render the correct number of project cards', async ({ page }) => {
  ...
  test('project cards should display title, category (if any), and image (if any)', async ({ page }) => {
    ...
    await expect(firstCard.locator(`.${'projectTitle'}`)).toHaveText('Petlove Quiz');
    await expect(firstCard.locator(`.${'projectCategory'}`)).toHaveText('Freelance');
    await expect(firstCard.locator(`.${'projectImage'}`)).toBeVisible();
  });

  test('project cards should have hover animation effects', async ({ page }) => {
    const firstCard = page.locator(`.${'projectCard'}`).first();
    const initialBoxShadow = await firstCard.evaluate(el => getComputedStyle(el).boxShadow);
    const initialScale = await firstCard.evaluate(el => getComputedStyle(el).transform);

    await firstCard.hover();
    await page.waitForTimeout(300); // Allow animation to complete

    const hoveredBoxShadow = await firstCard.evaluate(el => getComputedStyle(el).boxShadow);
    const hoveredScale = await firstCard.evaluate(el => getComputedStyle(el).transform);
  ...
  test('clicking a project card with an external link should open a new tab', async ({ page, context }) => {
    // Find a card that links externally, e.g., Portfolio Cookbook
    const cookbookCard = page.locator(`.${'projectCard'}:has-text("Portfolio Cookbook")`);
  ...
  test('project cards should be keyboard focusable and activatable', async ({ page }) => {
    const firstCard = page.locator(`.${'projectCard'}`).first();
    ...
    const focusOutline = await firstCard.evaluate(el => getComputedStyle(el).outlineStyle);
  ...
  test('grid should be responsive', async ({ page }) => {
    ...
    const initialColumns = await grid.evaluate(el => getComputedStyle(el).gridTemplateColumns.split(' ').length);
...
*(An animated GIF showcasing the project cards appearing with a stagger animation, the hover effects (scale, shadow), and the click interaction (navigating or opening a link) would be placed here.)*
...
const ProjectSummary = ({ title, shortDescription, isExpanded, onToggle, ariaControls }) => {
  return (
      ...
      {images && images.length > 0 && (
        <div className={styles.detailImages}>
          {images.map((src, index) => (
            <img key={index} src={src} alt={`Project detail ${index + 1}`} className={styles.detailImage} />
          ))}
        </div>
      )}
...
test.describe('Expandable Project Content Section', () => {
  ...
  test('should render the project summary initially collapsed', async ({ page }) => {
  ...
  test('should expand and show detail content on "Read More" click', async ({ page }) => {
    const firstSection = page.locator(`.${'expandableSection'}`).first();
    const readMoreButton = firstSection.locator(`button:has-text("Read More")`);
    ...
    await expect(firstSection.locator(`button:has-text("Read Less")`)).toBeVisible();
    await expect(firstSection.locator(`button:has-text("Read Less")`)).toHaveAttribute('aria-expanded', 'true');
  ...
  test('should collapse detail content on "Read Less" click', async ({ page }) => {
    const firstSection = page.locator(`.${'expandableSection'}`).first();
    const readMoreButton = firstSection.locator(`button:has-text("Read More")`);
    ...
    await expect(firstSection.locator(`button:has-text("Read More")`)).toBeVisible();
    await expect(firstSection.locator(`button:has-text("Read More")`)).toHaveAttribute('aria-expanded', 'false');
  ...
  test('multiple expandable sections should operate independently', async ({ page }) => {
    ...
    if (sections.length < 2) {
      console.warn('Skipping independent operation test: Less than 2 expandable sections found.');
      return;
    }
...
const projectDataExample = {
  ...
  summary: {
    ...
    shortDescription: 'Led company design and communications to explore numbers in narrative. Click to learn more about the challenges and solutions.',
  },
  detail: {
    longDescription: 'Detailed write-up about the Decipad project, covering user research, design iterations, collaboration with the team, and the impact of the design solutions. We focused on making data interaction intuitive and storytelling with numbers accessible...',
...
*(An animated GIF demonstrating a project summary with a "Read More" button. On click, the section smoothly expands downwards to reveal detailed text and images. Clicking "Read Less" collapses it.)*
